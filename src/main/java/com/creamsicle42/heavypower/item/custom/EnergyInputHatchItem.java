...
import com.creamsicle42.heavypower.blockentity.misc.IEnergyHatchManager;
...
public class EnergyInputHatchItem extends Item {
    public EnergyInputHatchItem(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (context.getLevel().getBlockEntity(context.getClickedPos()) instanceof SimpleMachinePartBlockEntity simpleMachinePartBlockEntity) {
            ...
            IEnergyHatchManager controller = (IEnergyHatchManager)
                    simpleMachinePartBlockEntity.getController().orElse(null);
            if (controller == null) {
                return super.useOn(context);
            }
            if (controller.tryMakeBlockEnergyInputHatch(context.getClickedPos(), context.getClickedFace())) {
